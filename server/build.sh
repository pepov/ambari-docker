...
docker run --privileged --name $INTERMEDIATE_NAME -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 8080:8080 -h ambari.server -d $INTERMEDIATE_IMAGE && \
docker exec -it $INTERMEDIATE_NAME bash -c "postgresql-setup initdb && systemctl enable postgresql && systemctl start postgresql && ambari-server setup -s && systemctl enable ambari-server" && \
docker stop $INTERMEDIATE_NAME && \
docker commit $INTERMEDIATE_NAME ambari/server:$RESULT_TAG
